...
if __name__ == "__main__":
    ...
    if args.data == "original":
        tokenized_fever = fever.map(tokenize_function, batch_size=512, batched=True)
        ...
        fever_train = fever_adv_train.map(tokenize_function, batch_size=512, batched=True)
        fever_valid = fever["validation"].map(tokenize_function, batch_size=512, batched=True)
        fever_test = load_dataset("matteo1822/fever_augmented_syn_hyper_hypo")["test"].map(tokenize_function, batch_size=512, batched=True)
